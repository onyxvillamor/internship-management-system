    ...
    if (
        ...
        if (empty($errors)) {
            try {
                ...
                $db->query(
                    'INSERT INTO internships (title, company_name, description, location, course, start_date, end_date, company_logo) 
                     VALUES (:title, :company_name, :description, :location, :course, :start_date, :end_date, :company_logo)',
                    [
                        ':title' => $title,
