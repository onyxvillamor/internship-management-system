...
<a href="/admin/students" class="inline-block m-4 px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400">Go back</a >
    <div class="max-w-6xl mx-auto">
   
    <?php if ($profile && is_array($profile)): ?> 
            <div class="flex flex-col md:flex-row mb-6 items-center md:items-start justify-start md:justify-center py-6">
                ...
                <div class="p-6">
                    ...
                    <?php if (isset($profile['company_name'])): ?>
                        ...
                        <ul class="list-disc pl-5">
                            <li><?= htmlspecialchars($profile['company_name']) ?> (<span class="<?= ($profile['status'] === 'Pending') ? 'text-yellow-500' : (($profile['status'] === 'Accepted') ? 'text-green-500' : (($profile['status'] === 'Rejected') ? 'text-red-500' : '')); ?>"><?= htmlspecialchars($profile['status']) ?></span>)</li>
                        </ul>
                    <?php else: ?>
